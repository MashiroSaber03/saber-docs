...
术语表和禁翻表是 Saber Translator 提供的两项翻译约束功能，按当前书籍或快速工作区保存。它们的作用不是直接替代翻译模型，而是帮助您在正式翻译前先告诉 AI 哪些词应该怎么翻，哪些内容不应该被翻译，从而提高整本漫画的统一性和可控性。
...
1. 从[书架章节](/use/bookshelf)或快速翻译入口进入翻译页面
2. 确认当前工作内容
3. 在左侧栏找到**术语表**和**禁翻表**按钮
4. 点击后分别打开对应的编辑窗口

::: tip 作用范围
书架章节使用所属书籍的约束；当前快速翻译工作区也提供“书籍约束”入口。约束不会自动应用到其他书籍。
:::
...
2. 命中的部分会被暂时替换为占位符
3. 文本输入中用占位符保护命中内容，降低误改概率；视觉模式仍会看到原图，保护结果也需要检查
